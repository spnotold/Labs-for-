...
def _make_client():
    ...
    """
    # === 在这里完成客户端初始化 ===
    api_key = os.getenv("DASHSCOPE_API_KEY") #利用OS库读取环境变量  
...
def _classify_one(client, model, content, temperature=0.0, max_retries=4):
    ...
    for attempt in range(max_retries):
        try:
            ...
            result_dict = _json_fix(raw_text)
            
            tag = result_dict.get("tag", "general_qa")
            ...
            if attempt < max_retries - 1:
                wait_time = 1.5 ** attempt * 1.5 
                time.sleep(wait_time)
            else:
                break
    
    # 所有重试都失败后的最终兜底
    return ("general_qa", "model_error: All retries failed.")
...
def tag_records(records, model="qwen-plus", show_progress=True):
    ...
    rows = []
    # 遍历 （用tqdm库显示进度）
    pbar = tqdm(records, desc="Processing records", disable=not show_progress)
    for r in pbar:
        #拼接 content
...
def main():
    ...
    # === 在这里调用单线程函数 ===
    print(f"开始使用模型 '{args.model}' 单线程处理 {len(records)} 条样本...")
    ...
    print("标签分布：", json.dumps(dist, indent=2))
